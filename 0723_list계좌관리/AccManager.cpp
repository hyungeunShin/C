...
AccManager::~AccManager()
{
	list<Account*>::iterator begin = base.begin();
	list<Account*>::iterator end = base.end();
	for (; begin != end; begin++)		
	{
		Account *pacc = *begin;
		delete pacc;		//힙메모리에 생성된 객체를 소멸
	}
}

void AccManager::Exist(int number)
{
	list<Account*>::iterator seek = base.begin();
	list<Account*>::iterator end  = base.end();
